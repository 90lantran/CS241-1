...
void pickup(phil_t *ps)
{
    ...
    int left = (ps -> id + (count -1)) % count; 
    int right = (ps -> id + 1) % count; 

    pthread_mutex_lock(global_state -> mon);
    global_state -> state[ps->id] = HUNGRY; 
    // WAITING FOR THE FORKS
    while (global_state -> state[left] == EATING ||global_state -> state[right] == EATING ){
      pthread_cond_wait(global_state -> cv[ps->id], global_state->mon);
    }
    //YOU HAVE 2 FORKS ATTACK

    global_state -> state[ps->id] = EATING; 
}
...
void putdown(phil_t *ps)
{
    ...
    pthread_mutex_lock(global_state ->mon);
    global_state->state[ps->id] = THINKING; 

    // LET OTHERS KNOW I AM DONE

    pthread_cond_signal(global_state -> cv[left]); 
    pthread_cond_signal(global_state -> cv[right]);
