...
int dictionary_parse(dictionary_t *d, char *key_value)
{
	...
	for(i = 0; i < len-1; i++){
		...
		if (key_value[i] == ':' && i != 0) {
			if (key_value[i+1] == ' '){
				key_value[i] = '\0'; 
				return  dictionary_add(d,key_value, &key_value[i+2]);
			}
...
{
	dictionary_entry_t * start; 
	
	for(start = d-> entry; start != NULL; start = start -> next){
		if (strcmp(start->key, key) ==0){return start -> value;}
	}
...
{
	if (dictionary_get(d, key) == NULL || d -> entry == NULL){
		return NO_KEY_EXISTS;
	}
	
	dictionary_entry_t * find; 
	
	for (find = d -> entry; find !=NULL; find = find -> next){
	...
	//Base Case 2: only 1 entry or removing the head
	
	if (strcmp(d -> entry -> key, find ->key) == 0){
		d -> entry = d -> entry -> next; 
		free(find);
		return 0; 
	}
	
	dictionary_entry_t * prev;
	
	for (prev = d ->entry; prev != NULL; prev = prev -> next){
		if (strcmp(prev -> next -> key, key)==0) {break;}
	}
...
{
	if (d -> entry != NULL){
		dictionary_entry_t * curr = d -> entry;
		d -> entry = curr -> next;
		free(curr);
